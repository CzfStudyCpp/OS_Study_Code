...
time_t getCurrentTime(void)
{
    ...
    current_dtime = (double) current_time.tv_sec +      //sec是秒  usec求小数位
...
comm_close(int fd)
{
	assert(fd>0);
	fde *F = &fd_table[fd];
	if(F) memset((void *)F,'\0',sizeof(fde));//将它清空
	epollSetEvents(fd, 0, 0);               //设置epoll事件设置为空
	close(fd);     //关闭描述符
}
...
commSetTimeout(int fd, int timeout, PF * handler, void *data)
{
    ...
    return F->timeout = sys_curtime + (time_t) timeout;//当前系统时间加上超时时间
}
